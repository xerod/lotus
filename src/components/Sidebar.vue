<template>
  <div ref="sidebar" v-if="showSidebar" class="px-4 pt-8 lg:pt-12">
    ...
    >
      <p
        class="pt-0 mt-0 mb-1 text-xs font-bold text-gray-500 uppercase border-none"
      >
        {{ section.title }}
      </p>
  ...
  computed: {
    ...
    sidebar() {
      return this.$static.metadata.settings.sidebar.find(
        sidebar => sidebar.name === this.$page.documentation.sidebar
      );
    },
    showSidebar() {
      return this.$page.documentation.sidebar && this.sidebar;
    },
    currentPage() {
